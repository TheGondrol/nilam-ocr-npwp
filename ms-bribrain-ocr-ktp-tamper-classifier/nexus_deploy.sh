#!/bin/bash
# Nexus deploy script (Bash) for ms-bribrain-ocr-ktp-tamper-classifier.
...
#
# Examples:
#   ./nexus_deploy.sh                # pulls ocr-ktp/dev/ms-bribrain-ocr-ktp-tamper-classifier:latest
#   ./nexus_deploy.sh stg v1.2.0     # pulls ocr-ktp/stg/ms-bribrain-ocr-ktp-tamper-classifier:v1.2.0
...
SERVICE_NAME="${SERVICE_NAME:-ms-bribrain-ocr-ktp-tamper-classifier}"

CONTAINER_NAME="${CONTAINER_NAME:-ms-bribrain-ocr-ktp-tamper-classifier}"
PORT_MAPPING="${PORT_MAPPING:-8050:8050}"
...
echo "Container $CONTAINER_NAME is running on http://localhost:8050"
echo "API Docs: http://localhost:8050/docs"
echo ""
echo "Test with: curl http://localhost:8050${HEALTH_PATH}"
