...
Usage:
    # In-process:
    pytest test/e2e/ -m e2e -v --no-cov

    # Against live server:
    E2E_BASE_URL=http://127.0.0.1:8050 pytest test/e2e/ -m e2e -v --no-cov
"""
...
def _collect_images() -> list[Path]:
    """Collect all image files from test/data/."""
    exts = {".jpeg", ".jpg", ".png"}
...
def image_path(request) -> Path:
    """Parametrised fixture yielding each image path in test/data/."""
...
async def e2e_client():
    ...
    if E2E_BASE_URL:
        ...
        mock_service.predict_from_bytes = AsyncMock(return_value={
            ...
            with patch("src.services.database_service.init_engine", new_callable=AsyncMock):
                with patch("src.services.database_service.dispose_engine", new_callable=AsyncMock):
                    with patch("src.api.routes.insert_log", new_callable=AsyncMock):
                        with patch("src.api.routes.get_tamper_service", return_value=mock_service):
                            with patch("src.services.minio_service.download_model_minio"):
                                with patch("src.services.gcs_service.download_model_gcs"):
                                    with patch("src.services.tamper_detection.initialize_tamper_service", return_value=mock_service):
