#!/bin/bash
# Nexus deploy script (Bash) for ms-bribrain-ocr-ktp-postprocessor.
...
#
# Examples:
#   ./nexus_deploy.sh                # pulls ocr-ktp/dev/ms-bribrain-ocr-ktp-postprocessor:latest
#   ./nexus_deploy.sh stg v1.2.0     # pulls ocr-ktp/stg/ms-bribrain-ocr-ktp-postprocessor:v1.2.0
...
CONTAINER_NAME="${CONTAINER_NAME:-ms-bribrain-ocr-ktp-postprocessor}"
PORT_MAPPING="${PORT_MAPPING:-8090:8090}"
...
echo "Container $CONTAINER_NAME is running on http://localhost:8090"
echo "API Docs: http://localhost:8090/docs"
echo ""
echo "Test with: curl http://localhost:8090${HEALTH_PATH}"
