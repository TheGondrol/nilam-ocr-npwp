"""
End-to-end tests for the Graycopy Classifier service.
...
actual ResNet-50 model. In in-process mode, the model is mocked but the
...
# ---------------------------------------------------------------------------
# Expected outcomes per image (for live server mode)
# ---------------------------------------------------------------------------

# graycopy.jpeg should be classified as GRAYCOPY by the real model
EXPECTED_GRAYCOPY_IMAGES = {"graycopy.jpeg"}

# All other images should be classified as ORIGINAL by the real model
...
class TestRootEndpoint:
    ...
    async def test_root(self, e2e_client):
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["message"] == "Graycopy Detection API"
...
class TestPredictionPipeline:
    """Test graycopy prediction on each image in tests/data/."""
    ...
    async def test_predict_returns_200(self, e2e_client, image_path, image_bytes, image_content_type):
        ...
        assert "probability_original" in data["data"]
        assert "probability_graycopy" in data["data"]
    ...
    async def test_prediction_values(self, e2e_client, image_path, image_bytes, image_content_type):
        ...
        pred = resp.json()["data"]
        assert pred["prediction"] in ("GRAYCOPY", "ORIGINAL")
        assert 0.0 <= pred["confidence"] <= 1.0
        assert 0.0 <= pred["probability_original"] <= 1.0
        assert 0.0 <= pred["probability_graycopy"] <= 1.0
...
class TestPredictionValidation:
    ...
    async def test_invalid_file_type(self, e2e_client):
        ...
        # The service may return 400 (image processing failed) or handle differently
