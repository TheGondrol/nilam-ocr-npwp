...
These tests call the REAL graycopy classifier service to verify detection
works correctly with an actual loaded ResNet-50 model.
...
Usage:
    ...
    # Against live server:
    E2E_BASE_URL=http://127.0.0.1:8040 pytest tests/e2e/ -m e2e -v --no-cov
"""
...
async def e2e_client():
    ...
    if E2E_BASE_URL:
        ...
        def mock_predict(image, filename):
            """Return a realistic graycopy prediction."""
            return {
                "prediction": "ORIGINAL",
                "confidence": 0.92,
                "probability_original": 0.92,
                "probability_graycopy": 0.08,
            }

        mock_svc.predict = mock_predict
                    ...
                    with patch("src.services.database_service.insert_log", new_callable=AsyncMock):
                        with patch("src.services.minio_service.download_model_minio"):
                            with patch("src.services.gcs_service.download_model_gcs"):
                                with patch("src.services.graycopy_detection.GraycopyDetectionService", return_value=mock_svc):
