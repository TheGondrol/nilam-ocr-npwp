# Graycopy Detection API

FastAPI service for detecting photocopied/graycopy ID documents vs original documents using a ResNet-50 deep learning model.

## Features

- **Binary Classification**: Detects GRAYCOPY (photocopied/scanned) vs ORIGINAL documents
- **Smart Preprocessing**: Auto-detects if images need cropping (target: 224x224)
...
```
ms-bribrain-ocr-ktp-graycopy-classifier/
├── src/
...
│   │   ├── graycopy_detection.py       # ResNet-50 classifier logic
│   │   ├── model_architecture.py       # ResNet-50 model definition
...
│   │   └── test_e2e_graycopy.py        # E2E prediction pipeline tests
│   └── data/                            # Test images
├── scripts/
...
# Model settings
model:
  path: ./models/graycopy_resnet50_best.pth
  image_size: 224
  normalize_size: 512
  dropout_rate: 0.6
...
device:
  ...
  jit_cpu: true                # JIT tracing for CPU inference
...
minio:
  ...
  object: ocr_ktp_models/graycopy_model.pth

# Model storage (cloud)
gcs:
  bucket_name: "gc-bribrain-dev-gcs-iguru-01"
  prefix: "ocr_models/graycopy_models/"
  model_prefix: "graycopy_model"
```
...
| `ELASTIC_APM_SERVICE_NAME` | No | APM service name | `ms-bribrain-ocr-ktp-graycopy-classifier` |
...
# Run the service
uv run uvicorn src.main:app --host 0.0.0.0 --port 8040
```
...
docker build -t ms-bribrain-ocr-ktp-graycopy-classifier .

# Build with e2e tests (requires DB and model storage to be reachable)
docker build --target e2e-test --network host --secret id=env,src=.env -t graycopy-test .

# Run the container
docker run -d \
  --name ms-bribrain-ocr-ktp-graycopy-classifier \
  --env-file .env \
  -p 8040:8040 \
  --restart always \
  ms-bribrain-ocr-ktp-graycopy-classifier
...
# View logs
docker logs -f ms-bribrain-ocr-ktp-graycopy-classifier
```
...
### POST /v1/ocr_graycopy

Classify whether an ID document image is a photocopy/graycopy or original.
...
```json
{
  ...
  "data": {
    ...
    "prediction": "ORIGINAL",
    "confidence": 0.9876,
    "probability_graycopy": 0.0124,
    "probability_original": 0.9876,
    "threshold": 0.5,
...
|------------|-------|-------------|
| `GRAYCOPY` | 0 | Photocopied or scanned document (suspicious) |
| `ORIGINAL` | 1 | Original legitimate document |
...
- **Architecture**: ResNet-50 with custom classification head (Dropout → Linear(2048→256) → ReLU → BatchNorm → Dropout → Linear(256→2))
- **Input**: RGB images, 224x224 pixels
- **Output**: Binary classification (GRAYCOPY vs ORIGINAL)
- **Preprocessing**: Smart auto-detection — if image is already 224x224, just normalize; otherwise Resize(512) → CenterCrop(224) → normalize
...
E2E_BASE_URL=http://127.0.0.1:8040 uv run pytest tests/e2e/ -m e2e -v
```
