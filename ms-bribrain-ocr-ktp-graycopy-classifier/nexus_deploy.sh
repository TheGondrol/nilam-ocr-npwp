#!/bin/bash
# Nexus deploy script (Bash) for ms-bribrain-ocr-ktp-graycopy-classifier.
...
#
# Examples:
#   ./nexus_deploy.sh                # pulls ocr-ktp/dev/ms-bribrain-ocr-ktp-graycopy-classifier:latest
#   ./nexus_deploy.sh stg v1.2.0     # pulls ocr-ktp/stg/ms-bribrain-ocr-ktp-graycopy-classifier:v1.2.0
...
SERVICE_NAME="${SERVICE_NAME:-ms-bribrain-ocr-ktp-graycopy-classifier}"

CONTAINER_NAME="${CONTAINER_NAME:-ms-bribrain-ocr-ktp-graycopy-classifier}"
PORT_MAPPING="${PORT_MAPPING:-8040:8040}"
...
echo ""
echo "Test with: curl http://localhost:8040${HEALTH_PATH}"
