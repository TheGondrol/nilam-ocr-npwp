...
def validate_config_values() -> Tuple[bool, List[str]]:
    ...
    # Validate device compile mode if compilation is enabled
    if config.device.compile_model and not config.device.is_valid_compile_mode:
        missing_configs.append(f"  - device.compile_mode (Invalid mode: '{config.device.compile_mode}')")
