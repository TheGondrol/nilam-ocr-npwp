...
def download_model_gcs() -> None:
    ...
    model_path = Path(config.get("model.path", "./src/models/qualitydl_model.pt"))

    # Short-circuit before any network call if a usable model is already on
    # disk, mirroring download_model_minio's exists-first ordering. Avoids a
    # spurious RuntimeError when the listing is empty/transient (BUG-17).
