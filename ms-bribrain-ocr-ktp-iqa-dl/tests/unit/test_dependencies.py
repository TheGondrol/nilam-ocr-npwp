...
class TestVerifyApiKey:
    ...
    async def test_api_key_leading_whitespace_rejected(self):
        """API key comparison is exact — leading whitespace is not stripped."""
        with patch.dict("os.environ", {"API_KEY": "secret"}):
            with pytest.raises(HTTPException) as exc_info:
                await verify_api_key(api_key=" secret")
            assert exc_info.value.status_code == 401

    async def test_api_key_is_case_sensitive(self):
