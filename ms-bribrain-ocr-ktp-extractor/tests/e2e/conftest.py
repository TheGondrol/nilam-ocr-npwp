...
Supports two modes:
  1. In-process (default): uses httpx ASGITransport with mocked OCR/DB.
...
Usage:
    ...
    # Against live server:
    E2E_BASE_URL=http://127.0.0.1:8010 pytest tests/e2e/ -m e2e -v --no-cov
"""
...
async def e2e_client():
    ...
    Otherwise, runs in-process with mocked OCR + DB.
    """
        ...
        with patch.dict(os.environ, {"API_KEY": api_key}):
            with patch("src.services.database_service.init_engine", new_callable=AsyncMock):
                with patch("src.services.database_service.dispose_engine", new_callable=AsyncMock):
                    with patch("src.main.is_ocr_ready", return_value=True):
                        with patch("src.api.routes.insert_log", new_callable=AsyncMock):
                            with patch("src.api.routes.perform_ocr_async", new_callable=AsyncMock, return_value=mock_ocr_result):
                                with patch("src.services.ocr_service.get_ocr"):
                                    with patch("src.services.ocr_service.warmup_ocr"):
