...
def render_pages(content_type: str | None, content: bytes, *, dpi: int, max_pages: int) -> list[Image.Image]:
    """An image is one page; a PDF is rendered page by page (at most `max_pages`). The type, size and page
    count of the document were checked by the orchestrator NPWP before it called this service."""
