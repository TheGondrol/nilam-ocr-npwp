...
| `ocr_npwp_requests` | **repo ini** | tidak ada | tidak ada | sisa kontrak lama sinkron (`generate-request-id` → `extract-ocr` → `get-ocr-result`) yang sudah dihapus dari ekstraksi. Definisinya tetap di `tables.py` supaya migrasi dan database yang ada tetap cocok; dibuang dengan migrasi tersendiri kalau sudah dipastikan tidak ada yang membaca isinya |
