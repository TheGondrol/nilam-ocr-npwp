...
class TestInsertLog:
    ...
    def _stub_encrypt(self):
        """insert_log now encrypts payload/result, which needs LOG_ENCRYPTION_KEY.
        Stub encrypt to a passthrough so these tests exercise the DB-logging logic."""
        with patch("src.services.database_service.encrypt", side_effect=lambda v: v):
            yield
