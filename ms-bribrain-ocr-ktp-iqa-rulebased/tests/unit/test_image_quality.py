...
def _all_checks_enabled():
    """Enable all quality checks regardless of config.yaml state."""
    with patch("src.services.image_quality.get_config") as mock_cfg:
        cfg = MagicMock()
        cfg.quality.confidence.enabled = True
        cfg.quality.blur.enabled = True
        cfg.quality.glare.enabled = True
        cfg.quality.rotation.enabled = True
...
class TestImageQuality:
    ...
    def test_invalid_image_bytes(self, sample_ocr_result):
        """Invalid (undecodable) image bytes must raise ImageDecodeError (→ 400)."""
        invalid_bytes = b"not an image"
        with pytest.raises(ImageDecodeError):
            image_quality(invalid_bytes, sample_ocr_result)

    def test_empty_image_bytes(self, sample_ocr_result):
        """Empty image bytes must raise ImageDecodeError (→ 400)."""
        with pytest.raises(ImageDecodeError):
            image_quality(b"", sample_ocr_result)
    ...
    def test_decode_failure(self, mock_imdecode, sample_ocr_result):
        """When cv2.imdecode returns None, raise ImageDecodeError (→ 400)."""
        mock_imdecode.return_value = None

        with pytest.raises(ImageDecodeError):
            image_quality(b"fake bytes", sample_ocr_result)
    ...
    def test_blur_detection_exception(
        ...
        """An unexpected orchestration failure propagates (→ 500).

        Detectors self-guard in production, so this mocks a detector raising to
        confirm image_quality no longer swallows such errors into a 'success'.
        """
        mock_blur.side_effect = Exception("Blur detection failed")

        with pytest.raises(Exception, match="Blur detection failed"):
    ...
    @patch('src.services.image_quality.get_config')
    @patch('src.services.image_quality.check_confidence_median')
    @patch('src.services.image_quality.blur_detection')
    @patch('src.services.image_quality.detect_glare_with_text_analysis')
    @patch('src.services.image_quality.detect_image_rotation')
    def test_all_checks_disabled(
        ...
        """When all checks disabled, returns all False without calling detection functions."""
        mock_config = MagicMock()
        mock_config.quality.confidence.enabled = False
        mock_config.quality.blur.enabled = False
        mock_config.quality.glare.enabled = False
        mock_config.quality.rotation.enabled = False
    ...
    @patch('src.services.image_quality.detect_glare_with_text_analysis')
    @patch('src.services.image_quality.detect_image_rotation')
    def test_blur_disabled_skips_blur_only(
        self,
        mock_rotation,
        mock_glare,
        mock_blur,
        ...
        """When blur is disabled, only blur_detection should not be called."""
        mock_config = MagicMock()
        mock_config.quality.confidence.enabled = True
        mock_config.quality.blur.enabled = False
        mock_config.quality.glare.enabled = True
        mock_config.quality.rotation.enabled = True
        mock_get_config.return_value = mock_config
    ...
    @patch('src.services.image_quality.detect_glare_with_text_analysis')
    @patch('src.services.image_quality.detect_image_rotation')
    def test_confidence_disabled_skips_confidence_only(
        self,
        mock_rotation,
        mock_glare,
        mock_blur,
        ...
        """When confidence is disabled, only check_confidence_median should not be called."""
        mock_config = MagicMock()
        mock_config.quality.confidence.enabled = False
        mock_config.quality.blur.enabled = True
        mock_config.quality.glare.enabled = True
        mock_config.quality.rotation.enabled = True
        mock_get_config.return_value = mock_config
    ...
    @patch('src.services.image_quality.detect_glare_with_text_analysis')
    @patch('src.services.image_quality.detect_image_rotation')
    def test_glare_disabled_skips_glare_only(
        self,
        mock_rotation,
        mock_glare,
        mock_blur,
        ...
        """When glare is disabled, only detect_glare_with_text_analysis should not be called."""
        mock_config = MagicMock()
        mock_config.quality.confidence.enabled = True
        mock_config.quality.blur.enabled = True
        mock_config.quality.glare.enabled = False
    ...
    @patch('src.services.image_quality.detect_glare_with_text_analysis')
    @patch('src.services.image_quality.detect_image_rotation')
    def test_rotation_disabled_skips_rotation_only(
        self,
        mock_rotation,
        mock_glare,
        mock_blur,
        ...
        """When rotation is disabled, only detect_image_rotation should not be called."""
        mock_config = MagicMock()
        mock_config.quality.confidence.enabled = True
        mock_config.quality.blur.enabled = True
        mock_config.quality.glare.enabled = True
    ...
    @patch('src.services.image_quality.detect_glare_with_text_analysis')
    @patch('src.services.image_quality.detect_image_rotation')
    def test_disabled_check_returns_false_while_enabled_detects_issue(
        self,
        mock_rotation,
        mock_glare,
        mock_blur,
        ...
        """Disabled checks return False even when enabled checks detect issues."""
        mock_config = MagicMock()
        mock_config.quality.confidence.enabled = True
        mock_config.quality.blur.enabled = False
        mock_config.quality.glare.enabled = True
        ...
        mock_confidence.return_value = True  # issue detected
        mock_glare.return_value = True  # issue detected

        result = image_quality(sample_image_bytes, sample_ocr_result)

        assert result["low_confidence"] is True
        assert result["is_blurry"] is False  # disabled, always False
        assert result["is_glare"] is True
        assert result["is_rotated"] is False  # disabled, always False

    def test_invalid_image_raises_decode_error(self, sample_ocr_result):
        """An undecodable image raises ImageDecodeError rather than degrading to all-False."""
        with pytest.raises(ImageDecodeError):
            image_quality(b"not an image", sample_ocr_result)
