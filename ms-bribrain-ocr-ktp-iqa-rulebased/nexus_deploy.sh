#!/bin/bash
# Nexus deploy script (Bash) for ms-bribrain-ocr-ktp-iqa-rulebased.
...
#
# Examples:
#   ./nexus_deploy.sh                # pulls ocr-ktp/dev/ms-bribrain-ocr-ktp-iqa-rulebased:latest
#   ./nexus_deploy.sh stg v1.2.0     # pulls ocr-ktp/stg/ms-bribrain-ocr-ktp-iqa-rulebased:v1.2.0
...
SERVICE_NAME="${SERVICE_NAME:-ms-bribrain-ocr-ktp-iqa-rulebased}"

CONTAINER_NAME="${CONTAINER_NAME:-ms-bribrain-ocr-ktp-iqa-rulebased}"
PORT_MAPPING="${PORT_MAPPING:-8070:8070}"
...
echo "Container $CONTAINER_NAME is running on http://localhost:8070"
echo "API Docs: http://localhost:8070/docs"
echo ""
echo "Test with: curl http://localhost:8070${HEALTH_PATH}"
