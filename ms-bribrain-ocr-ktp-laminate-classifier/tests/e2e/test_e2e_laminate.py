"""
End-to-end tests for the Laminate Classifier service.
...
actual CNN model. In in-process mode, the model is mocked but the
...
# unlaminated.jpeg should be classified as UNLAMINATED by the real model
EXPECTED_UNLAMINATED_IMAGES = {"unlaminated.jpeg"}

# All other images should be classified as LAMINATED by the real model
EXPECTED_LAMINATED_IMAGES = {
    "good_data.png",
    "classifier.jpeg",
    "graycopy.jpeg",
    "recapture.jpeg",
...
class TestRootEndpoint:
    ...
    async def test_root(self, e2e_client):
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["message"] == "Document Lamination Detection API"
...
class TestPredictionPipeline:
    """Test laminate prediction on each image in tests/data/."""
    ...
    async def test_prediction_values(self, e2e_client, image_path, image_bytes, image_content_type):
        ...
        pred = resp.json()["data"]
        assert pred["prediction"] in ("LAMINATED", "UNLAMINATED")
